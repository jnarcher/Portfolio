...
const ProjectCard = ({ data }: { data: Project }) => {
    return <div className="bg-neutral-800 p-3 rounded-md">
        ...
        {data.src && (
            <div className="flex justify-center items-center">
                <a className="flex justify-center items-center gap-2 hover:bg-neutral-700 px-3 py-1 rounded-md w-auto hover:text-white transition-colors" href={data.src} target="_blank">
                    <FaCode />
                    <span>Source</span>
                </a>
            </div>
...
const Tags = ({ data }: {data: string[]} ) => {
    return (
        <div className="flex gap-2">
            {data.map((tag, key) => (
                <div key={key} className="bg-neutral-700 px-2 py-1 rounded-lg">
                    {tag}
                </div>
            ))}
        </div>
